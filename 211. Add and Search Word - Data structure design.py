...
class WordDictionary:
    ...
    def search(self, word: str) -> bool:
        ...
        """
        return self.dict_trie.search_with_wild_card(word)
...
class Trie:
    ...
    def search_with_wild_card(self, word):
        cur = self.root
    ...
    def dfs(self, cur, word, index):
        if index == len(word):
            return cur.is_word
        for i in range(26):
            if (word[index] == '.' or ord(word[index]) - 97 == i) and cur.children[i] != None:
                if self.dfs(cur.children[i], word, index + 1):
                    return True
        return False
