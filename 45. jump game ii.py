...
class Solution3(object):
    def jump(self, nums):
        ...
        while len(queue):
            cur = queue.popleft()
            sofar_new = min(cur + nums[cur], len(nums) - 1)
            for i in range(farthest + 1, sofar_new + 1):
                f[i] = f[cur] + 1
                queue.append(i)
            farthest = max(farthest, sofar_new)
        return f[-1]
...
if __name__ == "__main__":
    ...
    A = [2,3,1,1,4] #2
    print(s.jump(A))
    # A = [17,8,29,17,35,28,14,2,45,8,6,54,24,43,20,14,33,31,27,11]#2
    # print(s.jump(A))
    # A = [2,3,1,1,4]#2
    # print(s.jump(A))
    # A = [1,1,1,1,1]#4
