...
class Solution(object):
    ...
    def numberToWords(self, num):
        ...
        while num > 0:
            if num % 1000 != 0:
                less_than_thousand = self.helper(num % 1000)
                words = less_than_thousand + self.THOUSANDS[i] + " " + words   # 第一次 helper(789)+''+''；第二次helper(456)+''+'Thousand'+算好的789；第二次helper(123)+''+'Million'+算好的45689；
            num //= 1000
            i += 1
        return words.strip()
    def helper(self,num):
