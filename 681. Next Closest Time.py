...
class Solution:
    def nextClosestTime(self, time: str) -> str:
        ...
        for perm in perms:
            if self.toMinutes(self.constructTime(perm)) - self.toMinutes(time) > 0 and self.toMinutes(
                    self.constructTime(perm)) - self.toMinutes(time) < minimum and self.isValid(
                    self.constructTime(perm)):
                nextClosest = True
